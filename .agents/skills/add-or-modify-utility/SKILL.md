...
That documentation should never include implementation details, such as used package names, unless it is absolutely necessary.
...
If the package is not in dependencies, it should be installed as a dev dependency and be added to `BUNDLED_PACKAGES` list in `meta.js` in the alphabetical order.
